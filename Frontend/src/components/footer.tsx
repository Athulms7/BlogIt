...
const Footer = () => {
  return (
    <section
      id="contact"
      className="relative bg-gradient-to-b from-[#0B071F] via-[#0F0826] to-[#07061A] py-24 px-4 overflow-hidden"
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <nav className="flex justify-center mb-8">
          <ul className="flex flex-wrap justify-center gap-x-10 gap-y-4">
            {["Home", "Our Platform", "Features", "FAQ", "Contact"].map(
              (item) => (
                <li key={item}>
                  <a
                    href="#"
        ...
        <p className="text-center text-white/60 font-light max-w-2xl mx-auto mb-10 text-lg tracking-normal">
          Empowering businesses with smart financial tools—secure, reliable, and
          built for growth.
        </p>

        {/* Social icons row */}
        <div className="flex justify-center gap-6 mb-16">
          {[FacebookLogoIcon, LinkedinLogoIcon, InstagramLogoIcon, XLogoIcon].map(
            (Icon, i) => (
              <div
                key={i}
